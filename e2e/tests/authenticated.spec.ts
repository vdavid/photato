...
test.describe('authenticated pages', () => {
  ...
  async function backdoorLogin(page: Page, request: APIRequestContext, email: string): Promise<void> {
    ...
    expect(response.ok(), `test-login failed with ${response.status()}`).toBeTruthy();
    const { sessionToken } = await response.json();
    await page.addInitScript((token) => window.localStorage.setItem('sessionToken', token), sessionToken);
  }
