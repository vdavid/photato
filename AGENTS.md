...
- **Cloudflare DNS move for `photato.eu`.** The zone still lives on Netlify DNS (all records already point at the box; the site works). Moving it to Cloudflare — for parity with David's other zones, managed as code in the `infra` repo `cloudflare/` — is blocked on two David-only steps: (1) create the zone (the shared `CLOUDFLARE_API_TOKEN` lacks `Zone:Create`), (2) switch nameservers at the registrar **ICDSoft** (no API/login creds stored). Keep every record DNS-only / grey-cloud so Caddy keeps doing its own TLS. Record set to replicate is in `docs/history/revival-plan.md` "Cloudflare DNS move".
