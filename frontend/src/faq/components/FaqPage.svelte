<script lang="ts">
    ...
    $effect(() => {
        document.title = __('Frequently asked questions') + ' - Photato'
    })
</script>
...
<div class="faqSummary">
    <ul>
        {#each questionsAndAnswers as questionAndAnswer (questionAndAnswer.id)}
            <!-- eslint-disable-next-line svelte/no-at-html-tags -- trusted: renders our own FAQ content, not user input -->
