...
export type TranslationMap = Partial<Record<string, Translation>>
...
export default class I18n {
  ...
  private _translations: Partial<Record<string, TranslationMap>>
  ...
  async loadTranslations(): Promise<void> {
    ...
    const localeAndTranslationPromises = this._availableLocaleCodes.map(async (localeCode) => ({
      localeCode,
      translations: await this._loadTranslationsForLocale(localeCode),
    }))
    const localesAndTranslations = await Promise.all(localeAndTranslationPromises)
    localesAndTranslations.forEach(
      (localeAndTranslation) =>
        (this._translations[localeAndTranslation.localeCode] = localeAndTranslation.translations),
    )
  }
  ...
  private _replacePlaceholdersInLocalizedString(localizedString: string, values: TranslationValues): string {
    return localizedString.replace(/{([\w-]+?)}/g, (_match, key: string) => (key in values ? String(values[key]) : key))
  }
  ...
  translate(phrase: string, values: TranslationValues = {}, localeCode: string = this._activeLocaleCode): string {
    const localeTranslations = this._translations[localeCode]
    if (localeTranslations) {
      const entry = localeTranslations[phrase]
      if (entry !== undefined) {
        return this._replacePlaceholdersInLocalizedString(entry.translation, values)
      } else {
        if (localeCode !== 'en-US') {
          /* We don't need translations for the base language. */
  ...
  private async _loadTranslationsForLocale(localeCode: string): Promise<TranslationMap> {
    ...
    const module = (await import(`./translations/${localeCode}.ts`)) as { translations: TranslationMap }
