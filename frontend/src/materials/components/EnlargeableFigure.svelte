<script lang="ts">
    ...
    const { fileName, thumbnailFileName, altText, caption }: Props = $props()
    ...
    const fullSizeImageUrl = $derived(imageBaseUrl + fileName)
    const thumbnailImageUrl = $derived(imageBaseUrl + (thumbnailFileName || fileName))
    ...
    const isFullscreen = $derived(fullscreenStatus !== fullscreenStatuses.notFullscreen)
    const imageSrc = $derived(!isFullscreen || !isFullSizeImagePreloaded ? thumbnailImageUrl : fullSizeImageUrl)
    ...
    $effect(() => {
        if (fullscreenStatus === fullscreenStatuses.goingToFullscreen) {
            ...
            downloadingImage.onload = () => {
                isFullSizeImagePreloaded = true
            }
            downloadingImage.onerror = (error) => {
                console.error(`Error loading ${fullSizeImageUrl}: ${(error as ErrorEvent).message}`)
            }
    ...
    const figureStyle = $derived.by(() => {
        ...
        if (fullscreenStatus === fullscreenStatuses.goingToFullscreen && figureRef) {
            ...
            if (html) {
                return `left: ${String(figureRef.offsetLeft - html.scrollLeft)}px; top: ${String(figureRef.offsetTop - html.scrollTop)}px; width: ${String(figureRef.offsetWidth)}px; height: ${String(figureRef.offsetHeight)}px`
            }
        }
        return ''
    })
