<script lang="ts">
    ...
    let {languageCode, slug, isExternalArticle = false}: Props = $props();
    ...
    $effect(() => {
        ...
        void (async () => {
            const module = (external
                ...
                : await import(`../own-content/${language}/${currentSlug}.svelte`)) as LoadedArticle;
...
{#if isLoaded && metadata && ArticleComponent}
    ...
    <article>
        <header>
            ...
            <p class="metadata articleMetadata">{__('Author') + ': ' + metadata.author} — {__('Publication date') + ': ' + metadata.publishDate.toLocaleDateString(getActiveLocaleCode())}{#if metadata.originalUrl} — <a href={metadata.originalUrl} target="_blank">{__('Original article')}</a>{/if}
            </p>
        </header>
        <Article/>
    </article>
