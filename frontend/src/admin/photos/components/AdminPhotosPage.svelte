<script lang="ts">
    ...
    $effect(() => {document.title = __('Photos') + ' - Photato admin';});
...
<div class="weekIndexSelector">
    {#each weekIndexes as week (week)}
        <a href="" data-value={week} onclick={(event) => {event.preventDefault(); weekIndex = (event.currentTarget as HTMLElement).getAttribute('data-value')!;}}>{week}</a>
    {/each}
</div>
<p>
