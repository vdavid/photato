...
export interface S3PhotoMetadataWire {
  ...
  url: string
  emailAddress: string
  /** Only present when the list was fetched with `includeTitleAndContentType`. */
  title?: string
  /** Only present when the list was fetched with `includeTitleAndContentType`. */
  contentType?: string
  sizeInBytes: number
...
export default class PhotoRemoteRepository {
  async getAllPhotosForWeek({
  ...
  }: GetAllPhotosForWeekOptions): Promise<S3PhotoMetadata[]> {
    const response = await fetch(
      url +
        ...
        convertObjectToQueryString({
          ...
          courseName,
          weekIndex,
          getDetails: includeTitleAndContentType ? 'true' : '',
        }),
      {
    ...
    const responseList = (await response.json()) as S3PhotoMetadataWire[]
    /* Revive the wire ISO string into a domain Date. */
    return responseList.map((photoMetadata) => ({
      ...photoMetadata,
      lastModifiedDate: new Date(photoMetadata.lastModifiedDate),
    }))
  }
}
