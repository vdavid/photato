<script lang="ts">
    ...
    $effect(() => {document.title = __('Sitemap generator') + ' - Photato Admin';});
    ...
    const pageInfosWithAbsoluteUrls = pageInfosWithRelativeUrls.map(pageInfo => ({...pageInfo, url: 'https://photato.eu' + pageInfo.relativeUrl}));
