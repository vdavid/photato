...
{#if message}
    ...
    <article>
        <header>
            ...
            <div class="metadata">
                <p>
                    Send via <strong>{message.channel}</strong>, to {message.locale}&nbsp;
                    <strong>{message.audience}</strong>. Content type is {message.contentType}.
                </p>
                <p>
                    <strong>Date/time: </strong>{getSendingTimeByDayIndex(message.courseDayIndex)} (Day {message.courseDayIndex}
                    of the course)
                </p>
