<script lang="ts">
    ...
    let {to, exact = false, class: className = '', activeClass = 'active', title, children, ...rest}: Props = $props();

    let active = $derived(isActive(to, {exact}));
    let computedClass = $derived([className, active ? activeClass : ''].filter(Boolean).join(' '));
