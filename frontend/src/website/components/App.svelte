<script lang="ts">
    ...
    interface RouteDef {
        pattern: string
        exact?: boolean
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- route table holds components with heterogeneous required props; a single precise type isn't expressible
    ...
    const routes: RouteDef[] = [
        ...
        {
            pattern: '/:languageCode/article/:slug',
            exact: true,
            component: MaterialPage,
            gate: 'public',
            extraProps: { isExternalArticle: false },
        },
        {
            pattern: '/:languageCode/external-article/:slug',
            exact: true,
            component: MaterialPage,
            gate: 'public',
            extraProps: { isExternalArticle: true },
        },
    ...
    const view = $derived.by(() => {
        if (!matched) {
            return {
                component: Error404Page as Component,
                props: {},
                parseEmoji: false,
            }
        }
        ...
        if (!allowed) {
            return { component: Error403Page as Component, props: {}, parseEmoji: false }
        }
        return {
            component: route.component,
            props: { ...params, ...(route.extraProps ?? {}) },
            parseEmoji: route.parseEmoji === true,
        }
    })
</script>
