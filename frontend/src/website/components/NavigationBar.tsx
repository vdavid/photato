...
import NavLinkMenuItemWithIcon from './NavLinkMenuItemWithIcon';
...
export default function NavigationBar() {
    ...
    const authenticationMenuRef = useRef<HTMLElement>(null);
    const {__} = useI18n();
    const location = useLocation();

    useEffect(() => {
        function hideMenusIfClickedOutside(event: MouseEvent) {
            if (menuRef.current && authenticationMenuRef.current
                && !menuRef.current.contains(event.target as Node) && !authenticationMenuRef.current.contains(event.target as Node)) {
