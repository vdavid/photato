<script lang="ts">
    ...
    $effect(() => {
        const timeout = setTimeout(() => {isTakingLong = true;}, 4000);
...
<div id="fullPageLoadingIndicator">
    ...
    {#if isTakingLong && areTranslationsLoaded()}
        <div class="loadingTakingLong">{@html __('Loading seems to take longer than usual. If you think this is a problem, please report it here.')}</div>
    {/if}
</div>
