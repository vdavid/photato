<script lang="ts">
    ...
    $effect(() => {
        ...
        return () => {
            document.removeEventListener('mousedown', hideMenuIfClickedOutside)
        }
    })
...
<header role="navigation">
    ...
    <nav bind:this={menuRef} class={isMenuVisible ? 'visible' : ''} onclick={() => (isMenuVisible = false)}>
        ...
        {#if auth.isAuthenticated}
            <NavLinkMenuItemWithIcon to="/course" iconName="casino"
                >{config.course.titleWithoutPhotato}</NavLinkMenuItemWithIcon
            >
        {/if}
        ...
        {#if auth.isAuthenticated}
            <a
                ...
                onclick={(event) => {
                    ...
                    void handleSignOut()
                }}><span class="icon material-icons">exit_to_app</span><span class="title">{__('Sign out')}</span></a
            >
        {/if}
    </nav>
