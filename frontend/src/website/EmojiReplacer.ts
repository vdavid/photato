...
type EmojiCallback = (icon: string, options: EmojiHow) => string
type EmojiAttributes = (rawText: string, iconId: string) => Record<string, string> | null

/** Normalized options used internally while parsing. */
interface EmojiHow {
  ...
  size: string
  className: string
  onerror: NonNullable<OnErrorEventHandler>
}

/** Caller-supplied options for `parse` — everything is optional and defaulted. */
...
interface EmojiReplacerOptions {
...
export default class EmojiReplacer {
  private readonly _defaultAssetsBaseUrl: string
  private readonly _defaultFileExtension: string
  private readonly _defaultSize: string
  private readonly _defaultClassName: string
  private readonly _emojiRegExp: RegExp

  /**
   * @param defaultAssetsBaseUrl If not specified, Twitter's URL will be used.
   * @param defaultFileExtension If not specified, ".png" will be used.
   * @param defaultSize If not specified, "72x72" will be used.
   * @param defaultClassName If not specified, "emoji" will be used.
   */
  constructor({
   ...
   *  // "\ud83c\udde8\ud83c\uddf3"
   */
  convertFromCodePoint(codePoint: string | number): string {
   ...
   *  // "1f1e8~1f1f3"
   */
  convertToCodePoint(unicodeSurrogates: string, separator = '-'): string {
   ...
   *
   */
  parse(what: string, options?: EmojiParseOptions): string
  parse(what: HTMLElement, options?: EmojiParseOptions): HTMLElement
  parse(
    what: string | HTMLElement,
    { callback, attributes, base, ext, folder, size, className, onerror }: EmojiParseOptions = {},
  ): string | HTMLElement {
    const createTextFunction = this._createText.bind(this)
    /**
     ...
     */
    function defaultErrorHandler(this: GlobalEventHandlers) {
      const image = this as unknown as HTMLImageElement
      if (image.parentNode) {
        image.parentNode.replaceChild(createTextFunction(image.alt, false), image)
      }
    }
     ...
     */
    function toSizeSquaredAsset(value: string | number): string {
      return typeof value === 'number' ? String(value) + 'x' + String(value) : value
    }

    const how: EmojiHow = {
      callback: callback || this._defaultImageSrcGenerator.bind(this),
   ...
   */
  replace(text: string, callback: (substring: string, ...args: unknown[]) => string): string {
    return text.replace(this._emojiRegExp, callback)
  }
   ...
   */
  private _createText(text: string, clean: boolean): Text {
   ...
   */
  private _defaultImageSrcGenerator(icon: string, options: EmojiHow): string {
   ...
   */
  private _grabAllTextNodes(node: Node | null, allText: Text[]): Text[] {
    ...
    if (!childNodes) return allText
    let length = childNodes.length
    let subNode: ChildNode
    let nodeType: number
    while (length--) {
   ...
   */
  private _grabTheRightIcon(rawText: string): string {
   ...
   */
  private _parseNode(node: HTMLElement, options: EmojiHow): HTMLElement {
    ...
    while (length--) {
      ...
      while ((match = this._emojiRegExp.exec(text))) {
        ...
        if (iconId && src) {
          ...
          for (attributeName in attrib) {
            if (
              Object.prototype.hasOwnProperty.call(attrib, attributeName) &&
      ...
      if (modified) {
        ...
        if (i < text.length) {
          fragment.appendChild(this._createText(text.slice(i), true))
        }
        // replace the text node only, leave intact
        // anything else surrounding such text
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- text node came from walking node's subtree, so it always has a parent
   ...
   */
  private _parseString(string: string, options: EmojiHow): string {
