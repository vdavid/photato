<script lang="ts">
    ...
    $effect(() => {document.title = __('12 weeks, 12 pics') + ' - Photato';});
</script>
...
<section class="frontPageSection frontPageMainCallToAction">
    <ExternalLink href={config.course.signUpFormUrl} class="callToActionButton">{__('Sign up for the next free course')}</ExternalLink>
</section>
...
<section class="frontPageSection threePoints">
    ...
    <div>
        <p>
            <span class="icon material-icons">photo</span><span class="icon material-icons">photo</span><span class="icon material-icons">photo</span>
        </p>
...
{#if getActiveLocaleCode() === 'hu-HU'}
    <section class="frontPageSection">
        <p>A tanfolyam ingyenes. Ha érdekel, kik csinálják ezt, és miért ingyen, a <Link to="/about">Rólunk</Link> oldalon további információkat találsz.
        </p>
        <p>Gyere fotózni, várunk! :)</p>
    </section>
{:else}
    <p>The course is free. If you’re interested in who does it and why it’s free, you can read more about us on the <Link to="/about">About page</Link>.
    </p>
    <p>Join us! :)</p>
{/if}
...
{:else}
    <section class="frontPageSection">
        ...
        <button onclick={() => navigate('/login')}>{__('Sign in')}</button>
    </section>
{/if}
