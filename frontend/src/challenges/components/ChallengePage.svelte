<script lang="ts">
    ...
    const { weekIndex }: { weekIndex: string } = $props()
    const weekIndexAsNumber = $derived(Number(weekIndex))

    const { currentWeekIndex, getDeadline } = courseData
    const languageCode = getActiveLocaleCode().substring(0, 2)
    const formattedDeadline = $derived(formatDateWithWeekDayAndTime(getDeadline(weekIndex), getActiveLocaleCode()))
    ...
    $effect(() => {
        if (weekIndexAsNumber >= 1 && currentWeekIndex >= weekIndexAsNumber) {
            ...
            void (async () => {
                try {
                    const module = (await import(
                        `../content/${languageCode}/Week${currentWeekIndexToLoad}Challenge.svelte`
                    )) as { default: ChallengeComponentType }
                    ChallengeComponent = module.default
                } catch {
            ...
            document.title =
                __('Week {weekIndex}:', { weekIndex }) +
                ' ' +
                __(weeklyChallengeTitles[weekIndexAsNumber - 1]) +
                ' - Photato'
        }
    })
</script>
    ...
    <article>
        ...
        {#if parseInt(weekIndex) === currentWeekIndex}
            <NavLinkButton
                ...
                title={!auth.isAuthenticated ? __('You’ll need to sign in to upload a photo.') : ''}
            >
