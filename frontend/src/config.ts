...
const { startDateTime, liveEventDate, exhibitionDate } = _calculateDates({
  startYear,
  startMonth,
  startDay,
  isDaylightSavingTimeOn,
...
export function initializeConfig(): void {
  ...
  const environmentSpecificConfig = host.startsWith('photato.eu')
    ? productionConfig
    : host.startsWith('staging.photato.eu')
      ? stagingConfig
      : developmentConfig
...
function _calculateDates({
  ...
  liveEventDate.setDate(
    // Latent bug (preserved, flagged for the owner): `+` binds tighter than `?:`, so this reads
    // `(getDate() + isWinterOrSummerCourse) ? A : B` and always picks the first branch. Kept as-is
    // to avoid silently shifting a shipped course date.
    String(liveEventDate.getDate()) + isWinterOrSummerCourse
