export default class OrientationFixer {
    ...
    _readFileToArrayBuffer(file: File): Promise<ArrayBuffer> {
        return new Promise((resolve, reject) => {
            ...
            fileReader.onloadend = (progressEvent: ProgressEvent<FileReader>) => {
                try {
                    resolve(progressEvent.target!.result as ArrayBuffer);
                } catch (error) {
                    reject(error);
                }
            };
     ...
     */
    _findUInt16InDataView(dataView: DataView, search: number, {start = 0, end = dataView.byteLength, isLittleEndian = false}: {start?: number; end?: number; isLittleEndian?: boolean} = {}): number | undefined {
        let index = start;
        while (index < end - 2) {
