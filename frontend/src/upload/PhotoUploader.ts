...
export default class PhotoUploader {
  ...
  async getSignedUrlFromServer(url: string, accessToken: string, parameters: SignedUrlParameters): Promise<string> {
    try {
      return await (await this._getSignedUrlFromServerOnce(url, accessToken, parameters)).text()
    } catch (error) {
      /* Try again once if 503 – this is because the lambda function tends to be slow the first time
  ...
  private _getSignedUrlFromServerOnce(
    url: string,
    accessToken: string,
    parameters: SignedUrlParameters,
  ): Promise<Response> {
  ...
  uploadFile(url: string, file: File, setUploadProgressCallback: (progress: number) => void): Promise<ProgressEvent> {
    return new Promise((resolve, reject) => {
      ...
      xmlHttpRequest.upload.addEventListener(
        'progress',
        (event) => {
          setUploadProgressCallback(event.loaded / event.total)
        },
        false,
      )
