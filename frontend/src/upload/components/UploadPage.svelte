<script lang="ts">
        ...
        return file.size > config.imageUpload.maximumSizeInBytes
            ...
            : file.size < config.imageUpload.minimumSizeInBytes
              ...
              : file.type !== 'image/jpeg'
                ? selectionStatuses.wrongFileType
    ...
    function getSelectionStatusText(status: SelectionStatus): string {
        ...
        const texts: Record<string, string> = {
            [selectionStatuses.readyToSelectFile.name]: __(
                'Please select your photo to upload. (a JPEG file of maximum 25 megabytes)',
            ),
            [selectionStatuses.selectedFileIsTooSmall.name]: __(
            ...
            [selectionStatuses.selectedFileIsTooLarge.name]: __(
            ...
            [selectionStatuses.wrongFileType.name]: __(
                'The image you’ve selected is not a JPEG. Please select a JPEG file.',
                { maximumSize },
            ),
            [selectionStatuses.readyToUpload.name]: __(
    ...
    function getUploadStatusText(status: SelectionStatus): string {
        const texts: Record<string, string> = {
            ...
            [uploadStatuses.uploadFailed.name]: __(
                ...
                { emailAddress: config.customerServiceEmailAddress },
            ),
        }
        return texts[status.name]
    }
    ...
    async function uploadSelectedFile() {
        // Capture the nullable state up front: the upload UI is disabled while a run is in
        // flight, so these can't change mid-upload, and const locals stay narrowed across awaits.
        const user = auth.user
        const file = selectedFile
        const token = accessToken
        if (!user || !file || token === null) {
            return
        }
        try {
            ...
            const signedUrl = await photoUploader.getSignedUrlFromServer(
                config.backendApi.photoUpload.url,
                token,
                parameters,
            )
            if (signedUrl) {
                const response = await photoUploader.uploadFile(signedUrl, file, (progress) => {
    ...
    const formattedDeadline = $derived(
        formatDateWithWeekDayAndTime(getDeadline(currentWeekIndex), getActiveLocaleCode()),
    )
    const courseStatusHelpText = $derived(
        isCourseRunning
            ? __(
...
<div id="fileUpload">
    ...
    {#if isCourseRunning}
        ...
        <div
            class={'uploadStatus' +
                (selectionStatus.isError ? ' error' : uploadStatus === uploadStatuses.uploadDone ? ' success' : '')}
        >
